export function validateBirthNumber(birthNumber: string) {
  birthNumber = birthNumber.replace(/[^0-9.]/g, "") // remove all characters but numbers
  ...
  //calculate checksum
  const number = Number([year, month, day, firstThreeDigitsOfSufix].join(""))
  if (length === 9 && number % 11 === 0) return true
  if (number % 11 === 10 && lastDigit === 0) return true
  if (number % 11 === lastDigit) return true
